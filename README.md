# Gerador de Senhas 

## 🔐 Sobre o Projeto

Este é um gerador de senha aleatória desenvolvido com HTML, CSS e JavaScript. Ele gera senhas seguras e aleatórias com base nos parâmetros fornecidos pelo usuário, como comprimento da senha e caracteres especiais.

## 🔎 Veja meu projeto online: 

👉🏽 https://generate-password-basic.netlify.app/

## 🛠️ Funcionalidades  

➡️ Geração de senhas aleatórias com opções personalizáveis.

➡️ Opções para incluir letras maiúsculas, minúsculas, números e caracteres especiais.

➡️ Botão para copiar a senha gerada para a área de transferência.

## ⚠️ Como utilizar

Não há pré-requisitos específicos para utilizar este gerador de senha.
